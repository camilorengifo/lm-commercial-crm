-- Create/replace bulk_update_company_fields for /api/companies/bulk-update.
-- companies.priority = public.company_priority
-- companies.account_status = TEXT
-- companies.sales_stage = TEXT
--
-- SECURITY INVOKER: respects existing RLS. Brokers update owned companies only.
...
BEGIN
  ...
  LOOP
    BEGIN
      -- RLS applies: brokers only see owned non-deleted rows; admins use admin policies.
      SELECT
        user_id,
        priority,
        sales_stage,
      ...
      -- Defense in depth (also enforced by RLS on UPDATE).
      ...
      -- Null params preserve existing values. updated_at is set by companies_set_updated_at trigger.
      ...
      -- Reuse existing commercial timeline (activities). Broker RLS requires user_id = auth.uid().
