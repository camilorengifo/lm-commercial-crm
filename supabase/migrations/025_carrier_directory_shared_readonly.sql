-- Lock shared Carrier Network mutations to admins while preserving broker create
-- and per-user user_carriers access.

-- ---------------------------------------------------------------------------
-- carriers: brokers create via RPC; only admins update/delete directly
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    IF NOT public.is_admin() THEN
      RAISE EXCEPTION 'Only admins can update shared carrier records';
    END IF;
...
DROP POLICY IF EXISTS carriers_insert_authenticated ON public.carriers;
DROP POLICY IF EXISTS carriers_update_authenticated ON public.carriers;
...
-- ---------------------------------------------------------------------------
-- Shared child tables: admin-only mutations; SELECT unchanged
-- ---------------------------------------------------------------------------

DROP POLICY IF EXISTS carrier_services_insert_authenticated ON public.carrier_services;
DROP POLICY IF EXISTS carrier_services_update_authenticated ON public.carrier_services;
DROP POLICY IF EXISTS carrier_services_delete_authenticated ON public.carrier_services;

CREATE POLICY carrier_services_insert_admin
ON public.carrier_services FOR INSERT TO authenticated
WITH CHECK (public.is_admin());

CREATE POLICY carrier_services_update_admin
ON public.carrier_services FOR UPDATE TO authenticated
USING (public.is_admin())
WITH CHECK (public.is_admin());

CREATE POLICY carrier_services_delete_admin
ON public.carrier_services FOR DELETE TO authenticated
USING (public.is_admin());

DROP POLICY IF EXISTS carrier_equipment_insert_authenticated ON public.carrier_equipment;
DROP POLICY IF EXISTS carrier_equipment_update_authenticated ON public.carrier_equipment;
DROP POLICY IF EXISTS carrier_equipment_delete_authenticated ON public.carrier_equipment;

CREATE POLICY carrier_equipment_insert_admin
ON public.carrier_equipment FOR INSERT TO authenticated
WITH CHECK (public.is_admin());

CREATE POLICY carrier_equipment_update_admin
ON public.carrier_equipment FOR UPDATE TO authenticated
USING (public.is_admin())
WITH CHECK (public.is_admin());

CREATE POLICY carrier_equipment_delete_admin
ON public.carrier_equipment FOR DELETE TO authenticated
USING (public.is_admin());

DROP POLICY IF EXISTS carrier_service_areas_insert_authenticated ON public.carrier_service_areas;
DROP POLICY IF EXISTS carrier_service_areas_update_authenticated ON public.carrier_service_areas;
DROP POLICY IF EXISTS carrier_service_areas_delete_authenticated ON public.carrier_service_areas;

CREATE POLICY carrier_service_areas_insert_admin
ON public.carrier_service_areas FOR INSERT TO authenticated
WITH CHECK (public.is_admin());

CREATE POLICY carrier_service_areas_update_admin
ON public.carrier_service_areas FOR UPDATE TO authenticated
USING (public.is_admin())
WITH CHECK (public.is_admin());

CREATE POLICY carrier_service_areas_delete_admin
ON public.carrier_service_areas FOR DELETE TO authenticated
USING (public.is_admin());

DROP POLICY IF EXISTS carrier_contacts_insert_authenticated ON public.carrier_contacts;
DROP POLICY IF EXISTS carrier_contacts_update_authenticated ON public.carrier_contacts;
DROP POLICY IF EXISTS carrier_contacts_delete_authenticated ON public.carrier_contacts;

CREATE POLICY carrier_contacts_insert_admin
ON public.carrier_contacts FOR INSERT TO authenticated
WITH CHECK (public.is_admin() AND created_by = auth.uid());

CREATE POLICY carrier_contacts_update_admin
ON public.carrier_contacts FOR UPDATE TO authenticated
USING (public.is_admin())
WITH CHECK (public.is_admin());

CREATE POLICY carrier_contacts_delete_admin
ON public.carrier_contacts FOR DELETE TO authenticated
USING (public.is_admin());

-- ---------------------------------------------------------------------------
-- Broker create: SECURITY DEFINER bundle (bypasses admin-only RLS)
-- ---------------------------------------------------------------------------
